  ...
  module Core
    ...
    class ModelAssociations
      ...
      def get_association_class_name(table_name, association_name)
        associations = @associations.reject{ |model, model_associations| model.gsub("::", "").tableize != table_name }.values.first and
          association_meta = associations.reject { |name, meta| name != association_name }.values.first and
